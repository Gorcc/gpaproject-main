...
const des = () => {
  ...
  DES.setParityBits = function (bitArray) {
    for (let bitIndex = 0; bitIndex < bitArray.length; bitIndex += 8) {
      ...
      for (
        let subBitIndex = bitIndex;
        subBitIndex < bitIndex + 7;
        subBitIndex++
      ) {
  ...
  DES._encryptOrDecrypt = function (key, input, mode) {
    ...
    result.initialPermutation = DES.permute(
      result.input,
    ...
    result.rounds = DES._doRounds(
      result.roundKeys.roundKeyParts,
      result.initialPermutation,
      DES.MODE.ENCRYPTION
    );

    result.finalRoundSwitch = result.rounds[
      DES.NUM_ROUNDS - 1
    ].rightFinal.concat(result.rounds[DES.NUM_ROUNDS - 1].leftFinal);

    result.final = DES.permute(
      result.finalRoundSwitch,
  ...
  DES._generateKeys = function (inputKey) {
    ...
    result.initialKeyPermutations = {
      c: DES.permute(inputKey, DES.PERMUTATION_MAPPINGS.PC1_C),
      d: DES.permute(inputKey, DES.PERMUTATION_MAPPINGS.PC1_D),
    };
    ...
    for (let roundIndex = 0; roundIndex < DES.NUM_ROUNDS; roundIndex++) {
      ...
      keyParts.pc2C = DES.permute(
        keyParts.combinedCD,
        DES.PERMUTATION_MAPPINGS.PC2_C
      );
      keyParts.pc2D = DES.permute(
        keyParts.combinedCD,
  ...
  DES._doRounds = function (roundKeys, permutedInput, mode) {
    ...
    for (let roundIndex = 0; roundIndex < DES.NUM_ROUNDS; roundIndex++) {
      ...
      if (mode === DES.MODE.ENCRYPTION) {
        thisRound.manglerData = DES._mangle(
          roundKeys[roundIndex].key,
          thisRound.rightInitial
        );
  ...
  DES._mangle = function (roundKey, input) {
    ...
    manglerData.expandedData = DES.permute(
      input,
    ...
    manglerData.finalOutput = DES.permute(
      manglerData.combinedOutputs,
      DES.PERMUTATION_MAPPINGS.SBOX_PERMUTATION
    );
  ...
  DES._processSBoxChunks = function (manglerData) {
    ...
    for (let sBoxIndex = 0; sBoxIndex < 8; sBoxIndex++) {
      let rowColumn = DES._getSBoxRowAndColumn(
        manglerData.sBoxFinalInputs[sBoxIndex]
      );

      let sBoxDataIndex = rowColumn.row * 16 + rowColumn.column;

      manglerData.sBoxOutputs[sBoxIndex] = BitArray.fromNumber(
        DES.SBOXES[sBoxIndex][sBoxDataIndex],
        4
      );
  ...
  DES._getSBoxRowAndColumn = function (input) {
    ...
    result.column =
      (columnBit1 << 3) | (columnBit2 << 2) | (columnBit3 << 1) | columnBit4;
